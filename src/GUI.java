...
cards.add(new Cards(i,shape1,setId));
setId++;

}
}
randomNumber = new Random().nextInt(52);
playerCards.add(cards.get(randomNumber));
cards.get(randomNumber).used = true;


randomNumber = new Random().nextInt(52);
while(true)
{
if(cards.get(randomNumber).used==false)
{
dealerCards.add(cards.get(randomNumber));
cards.get(randomNumber).used = true;
break;
}
else
...
{
	System.out.println(("Player has card"+" "+c.name+" "+"of"+" "+c.shape));
	char shape = c.shape.charAt(0);
	char lShape = Character.toLowerCase(shape);
	String path = "test/"+c.num+lShape+".gif";
...
{
System.out.println("Dealer has card"+" "+c.name+" "+"of"+" "+c.shape);
char shape = c.shape.charAt(0);
char lShape = Character.toLowerCase(shape);
String path = "test/"+c.num+lShape+".gif";
System.out.println(path);
